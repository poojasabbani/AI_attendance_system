    ...
    with btnc1:
        if st.button("login",icon=':material/passkey:',shortcut="control+enter",width="stretch"):
            if login_teacher(teacher_username,teacher_pass):
                ...
                st.error("Username and password mismatch!")
...
def register_check(teacher_username,teacher_name,teacher_pass,teacher_pass_confirm):
    ...
    try:
        ...
        return True, "Sucessfully Registered!!! Login Now"
...
def teacher_screen_register():
    ...
    with btnc1:
        if st.button("Register Now",icon=':material/passkey:',shortcut="control+enter",width="stretch"):
            success,message = register_check(teacher_username,teacher_name,teacher_pass,teacher_pass_confirm)
            if success:
                st.success(message)
                import time
                time.sleep(2)
